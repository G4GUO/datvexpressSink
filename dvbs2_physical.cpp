...
void DVBS2::s2_b_64_7_code( unsigned char in, int *out )
{
...
//[MODCOD 6:2 ][TYPE 1:0 ]
void DVBS2::s2_pl_header_encode( uint8_t modcod, uint8_t type, int *out)
{
    uint8_t code;
...
int  DVBS2::s2_pl_data_pack( void )
{
    ...
    s2_pl_scramble_symbols( &m_pl[90], n - 90 );
    // Return the length
    return n;
}
//
...
//
void DVBS2::s2_pl_build_dummy( void )
{
    int n = 0;
    int b[90];
    ...
    n += (90*36);
    s2_pl_scramble_dummy_symbols( n );
    m_dummy_frame_length = n;
}

SComplex * DVBS2::s2_pl_get_frame( void )
{
    return m_pl;
}

SComplex * DVBS2::s2_pl_get_dummy( int &len )
{
    SComplex * frame;
